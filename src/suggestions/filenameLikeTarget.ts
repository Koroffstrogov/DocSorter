...
export function isFilenameLikeTarget(
  ...
  if (documentType && normalized.includes(documentType) && (YEAR_PATTERN.test(normalized) || TEST_PREFIX_PATTERN.test(normalized))) {
    return true;
  }
...
export function isFallbackManualFolder(relativePath: string | undefined): boolean {
  return normalizeNameBlock(relativePath?.replace(/[\\/]+/g, " ") ?? "") === "divers-a-traiter-manuellement";
}
